    ...
    if (userType === 'vendor') {
      try {
        const res = await authAPI.registerVendor({
          ...
          shopName: data.businessName, businessType: data.businessType,
          phone: data.phone, address: data.address,
        });
        result = res.data.success ? { success: true } : { success: false, error: res.data.message };
      } catch (err) {
        result = { success: false, error: err.response?.data?.message || err.message };
      }
    } else {
          ...
          <Box component="form" onSubmit={handleSubmit} sx={{ width: '100%' }}>
            ...
            {userType === 'vendor' && (
              <>
                ...
                <TextField margin="normal" required fullWidth select name="businessType" label="Business Type" value={formData.businessType} onChange={handleChange} error={!!formErrors.businessType} helperText={formErrors.businessType}>
                  {['Food', 'Drinks', 'Clothing', 'Electronics'].map((t) => <MenuItem key={t} value={t}>{t}</MenuItem>)}
                </TextField>
                ...
                <TextField margin="normal" required fullWidth multiline rows={2} name="address" label="Address" value={formData.address} onChange={handleChange} error={!!formErrors.address} helperText={formErrors.address} />
              </>
            )}
