...
api.interceptors.request.use(
  (config) => {
    ...
    // Only skip auth for truly public / non-API paths (favicon, manifest, etc.)
    const isPublicAsset =
    ...
    if (!isPublicAsset) {
      ...
      console.log(`[Auth] Skipping auth for asset: ${config.url}`);
    }
...
export const productAPI = {
  ...
  getMyReview: (id) => api.get(`/products/${id}/reviews/my`),
  addReview: (id, rating, reviewText) => api.post(`/products/${id}/reviews`, { rating, reviewText }),
